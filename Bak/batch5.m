...
for s=1:numel(subjects)
  ...
  if exist([outdir 'ica_libcomps.txt'],'file')
    libcomps = load([outpath 'ica_libcomps.txt']);
    rejcomps = union(rejcomps, libcomps);
  else
      disp('LibComps doesnt exist');
  end
  ...
  for trial=1:EEG.trials
    ...
    trial_keepcomps = setdiff([1:size(icaact,1)],trial_rejcomps);
    % clean up part of the data that is coverd by ica and add back leftover signal
    EEG.data(:,:,trial) =  (EEG.icawinv(:,trial_keepcomps) * icaact(trial_keepcomps,:,trial)) + (EEG.data(:,:,trial) - EEG.icawinv * icaact(:,:,trial));
  end
