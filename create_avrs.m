    ...
    for c=1:numel(merged_conditions)
        ...
        for j=1:numel(merged_conditions{c})
           ...
           % data sets with different numbers of trials
            ...
            fprintf(fid,'Npts= %d TSB= %d  DI= %d  SB= 1.000  SC= 200 Nchan= %s\n',EEG.pnts, EEG.xmin*100, EEG.pnts/EEG.srate, nchans);
        ...
        % if all you want is averages of each individual condition per subject, you
        % don't need the rest of this code - this next section produces an overall
        % average for a given subject across all conditions (i.e. per "merged condition").
        ...
        fprintf(fid, '%s %d\n', ['Npts= 2400  TSB= -200.0  DI= 0.5  SB= 1.000  SC= 200 Nchan='],nchans);
